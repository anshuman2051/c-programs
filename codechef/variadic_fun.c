...
int min(int count,...) {
    int min = 86400;
    va_list arg_ptr;    //va_list declares pointer
    va_start( arg_ptr, count); 
      //va_start function links the pointer to that of variables , 
      //it takes total no of variables
    int val;
    for( int i = 0; i < count; i++){
        val = va_arg(arg_ptr, int) ;        //va_args gives value of each var, given its type
        if( val < min ) min = val;
    }
    va_end(arg_ptr);            //it resets or ends the pointer;
    return min;
}
